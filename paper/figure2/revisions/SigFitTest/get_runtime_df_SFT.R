...
times_sigpr = read.csv(file.path(runtime_path, "sigprofiler_exectimes.csv"), header=F) %>% 
  tidyr::separate("V1", into=c("simulation_name", "execution_time"), sep="/: T") %>% 
  rowwise() %>%
  mutate(simulation_name=case_when(
    grepl("/: ", simulation_name) ~ strsplit(simulation_name, "/: ")[[1]][2],
    .default=simulation_name
    )) %>% 
  ungroup() %>% 
  mutate(execution_time=stringr::str_remove_all(execution_time, ": Total execution time: | ")) %>% 
  mutate(execution_time=lubridate::period_to_seconds(lubridate::hms(execution_time)) / 60) %>% 
  mutate(tool="SigProfiler") %>% tibble::as_tibble()

times_sparsesig = read.csv(file.path(runtime_path, "sparsesignatures_exectimes.csv"), header=F) %>% 
  rename(execution_time=V5, simulation_name=V1) %>% 
...
times_bascule = read.csv(file.path(runtime_path, "bascule_exectimes.csv")) %>% 
  ...
  mutate(simulation_name=stringr::str_remove_all(simulation_name, "fits_dn.matched.2011.compare_LAST/")) %>% 
...
saveRDS(runtime_df, file.path(save_path, "runtime_SigFitTest.Rds"))
