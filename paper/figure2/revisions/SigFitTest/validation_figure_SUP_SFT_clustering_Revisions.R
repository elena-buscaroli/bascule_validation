...
pvalues = ggpubr::compare_means(nmi ~ Method, data=input_df, method="wilcox.test", exact=FALSE, 
                                group.by=c("N","source","nMuts_label","nMuts")) %>%
  filter(group1=="BASCULE" | group2=="BASCULE") %>% 
  group_by(N, source, nMuts_label, nMuts) %>% 
...
clustering = input_df %>%
  ...
  ggpubr::stat_pvalue_manual(
    pvalues %>% ungroup(), label="p.signif", x="N", color="group1",
...
ggsave(filename="paper/figure2/revisions/SigFitTest/figure2_SUP7.png", plot=figure,
       width=210, height=150, units="mm", device=png, family="Helvetica")
