...
all_stats %>% 
  ...
  dplyr::select(dplyr::all_of(id_cols), K_input_ratio) %>% 
  ggplot() +
  geom_violin(aes(x=factor(N), y=K_input_ratio)) +
...
all_stats %>% 
  dplyr::filter(penalty=="Autoguide") %>% 
  dplyr::rowwise() %>% 
  dplyr::mutate(K_dn_found=length(intersect(assigned_missing$assigned_tp, dn_sigs)),
                K_dn_true=K_true - length(input_sigs),
                K_dn_ratio=K_dn_found / K_dn_true) %>% 
  dplyr::select(dplyr::all_of(id_cols), K_dn_ratio) %>% 
  ggplot() +
  geom_violin(aes(x=factor(N), y=K_dn_ratio)) +
  geom_hline(yintercept=1, color="grey70", linetype="dashed") +
  facet_grid(~type) +
  theme_bw()
...
all_stats %>% 
  dplyr::filter(penalty=="Autoguide") %>% 
  dplyr::rowwise() %>% 
  dplyr::mutate(K_found=length(assigned_missing$assigned_tp),
                K_ratio=K_found / K_true) %>% 
  dplyr::select(dplyr::all_of(id_cols), K_ratio) %>% 
  ggplot() +
  geom_violin(aes(x=factor(N), y=K_ratio)) +
  geom_jitter(aes(x=factor(N), y=K_ratio), size=0.5, width=0.2, height=0.01) +
  geom_hline(yintercept=1, color="grey70", linetype="dashed") +
  facet_grid(~type) +
  theme_bw() + ylim(0,NA)
...
all_stats %>% 
  ...
  # dplyr::filter(grepl("^mse|^cosine",metric), penalty=="Autoguide") %>% 
  dplyr::filter(metric %in% metrics, penalty=="Autoguide") %>% 
  tidyr::separate("metric", into=c("metric","variable"), extra="merge", sep="_") %>% 
  ggplot() +
  geom_boxplot(aes(x=factor(N), y=value)) +
  ggh4x::facet_nested(metric+variable~type, scales="free_y") +
  theme_bw()
...
all_stats %>% 
  ...
  dplyr::group_by(N, metric, penalty) %>% 
  ggplot() +
  geom_boxplot(aes(x=factor(N), y=filter_lims(value), fill=penalty), na.rm=T) +
  geom_hline(yintercept=1, color="grey70", linetype="dashed") +
