...
main_path = "/orfeo/scratch/cdslab/ebusca00/signatures/fits_SigFitTest/"
fits_path = file.path(main_path, "fits_dn.SigFitTest.FAST/")
sp_path = file.path(main_path, "sigprofiler_BestSolution/")
ss_path = file.path(main_path, "sparsesignatures/signatures/")
...
lapply(fitsnames, function(fname) {
  ...
  try({
    tmp = stringr::str_replace_all(fname, ".Rds", "")
    ss_fname = list.files(paste0(ss_path, tmp), pattern=".Rds", full.names=TRUE)
    ss_res = readRDS(ss_fname)
